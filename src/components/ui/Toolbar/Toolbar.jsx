...
function Toolbar({showAppWindow, showCardDetails, launchButtonStartSelected, buttonLaunch_selecedLabel, buttonLaunch_Label,
                  addNewBackup, toggleCheckAllbCards, toggleCheckAll, toolbar_showDeleteIcon, deleteSelectedBackupWarn, toolbar_showSelectAllIcon})
{
  return(
    <div className={classNames('Toolbar ', {'Toolbar--active': showAppWindow & !showCardDetails , '' : !showAppWindow })}>
      ...
      <div className="functionButton-container dFlex">
        ...
        <button className="functionButton button-deleteBackup" disabled={toolbar_showDeleteIcon? null : 'disabled'} onClick={() => {deleteSelectedBackupWarn()}}><Icon name="trash" color="" size={20} /></button>
      </div>
    </div>
  )
}
